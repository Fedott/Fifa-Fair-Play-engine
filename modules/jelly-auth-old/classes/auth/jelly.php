...
class Auth_Jelly extends Auth {
	...
	public function auto_login()
	{
		if ($token = Cookie::get('authautologin'))
		{
			...
			$token = Jelly::query('user_token')->where('token', '=', $token)->execute();
	...
	public function logout($destroy = FALSE, $logout_all = FALSE)
	{
		if ($token = Cookie::get('authautologin'))
		{
			...
			$token = Jelly::query('user_token')->where('token', '=', $token)->limit(1)->execute();
	...
	protected function _get_object($user)
	{
		...
		{
			// Load the user
			$current = Jelly::query('user')->where('username', '=', $user)->limit(1)->execute();
		}
